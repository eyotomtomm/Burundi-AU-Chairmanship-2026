...
class WaterSanitationScreen extends StatefulWidget {
  ...
  @override
  State<WaterSanitationScreen> createState() => _WaterSanitationScreenState();
}

class _WaterSanitationScreenState extends State<WaterSanitationScreen> {
  final ApiService _apiService = ApiService();
  Map<String, dynamic>? agendaData;
  bool isLoading = true;

  // Theme colors for Water & Sanitation
  static const _primary = Color(0xFF0077B6);
  static const _primaryLight = Color(0xFF00B4D8);
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_tList('objectives').isNotEmpty) ...[
                              _buildSectionHeader(
                                _isFr ? 'Objectifs Clés' : 'Key Objectives',
                                Icons.flag_rounded,
                                isDark,
                              ),
                              const SizedBox(height: 12),
                            ...
                            if (_tList('impact_areas').isNotEmpty) ...[
                              _buildSectionHeader(
                                _isFr ? "Domaines d'Impact" : 'Impact Areas',
                                Icons.hub_rounded,
                                isDark,
                              ),
                              const SizedBox(height: 12),
                            ...
                            if (_t('current_initiatives').isNotEmpty) ...[
                              _buildSectionHeader(
                                _isFr ? 'Initiatives en Cours' : 'Current Initiatives',
        ...
        background: Stack(
          ...
          children: [
            ...
            Positioned(
              right: 24,
              bottom: 64,
              child: Icon(Icons.water_drop, size: 60, color: Colors.white.withValues(alpha: 0.15)),
            ),
          ],
        ),
      ),
        ...
        Expanded(
          child: Text(
            ...
            style: TextStyle(
              fontSize: 22,
              fontWeight: FontWeight.w800,
              color: isDark ? const Color(0xFF4FC3F7) : _primary,
              letterSpacing: -0.3,
            ),
          ),
        ),
